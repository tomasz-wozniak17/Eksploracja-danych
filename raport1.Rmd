...
p1 <- ggplot(dane, aes(x = Churn, fill = Churn)) +
  ...
  labs(title = "Rezygnacja z usług (Churn)", x = "Czy klient odszedł?", y = "Liczba klientów") +
  theme_minimal() +
  theme(legend.position = "none")

# 2. Rodzaj umowy (Contract)
p2 <- ggplot(dane, aes(x = Contract, fill = Contract)) +
  geom_bar(color = "black") +
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) +
  scale_fill_manual(values = c("mistyrose", "pink", "hotpink")) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  labs(title = "Rodzaj zawartej umowy", x = "Typ kontraktu", y = "Liczba klientów") +
  theme_minimal() +
  theme(legend.position = "none")

# 3. Rodzaj internetu (InternetService)
p3 <- ggplot(dane, aes(x = InternetService, fill = InternetService)) +
  geom_bar(color = "black") +
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) +
  scale_fill_manual(values = c("palevioletred1", "maroon1", "maroon3")) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2))) + 
  labs(title = "Dostarczane usługi internetowe", x = "Technologia", y = "Liczba klientów") +
  theme_minimal() +
  theme(legend.position = "none")
...
```{r rozklady_ilosciowe_grupy, echo=FALSE, warning=FALSE, message=FALSE, fig.height=8, fig.width=6, fig.align='left'}
# 1. Staż a rezygnacja 
g1 <- ggplot(dane, aes(x = tenure, fill = Churn)) +
  geom_density(alpha = 0.6, color = "black") + # alpha = 0.6 daje półprzezroczystość
  scale_fill_manual(values = c("No" = "lightpink", "Yes" = "deeppink")) +
  labs(title = "Rozkład stażu w podziale na grupy", x = "Staż (miesiące)", y = "Gęstość") +
  theme_minimal()

# 2. Opłaty miesięczne a rezygnacja
g2 <- ggplot(dane, aes(x = MonthlyCharges, fill = Churn)) +
  geom_density(alpha = 0.6, color = "black") +
  scale_fill_manual(values = c("No" = "lightpink", "Yes" = "deeppink")) +
  labs(title = "Rozkład opłat mies. w podziale na grupy", x = "Opłata miesięczna (USD)", y = "Gęstość") +
  theme_minimal()

# 3. Opłaty całkowite a rezygnacja
g3 <- ggplot(dane, aes(x = TotalCharges, fill = Churn)) +
  geom_density(alpha = 0.6, color = "black") +
  scale_fill_manual(values = c("No" = "lightpink", "Yes" = "deeppink")) +
  labs(title = "Rozkład opłat całk. w podziale na grupy", x = "Opłata całkowita (USD)", y = "Gęstość") +
  theme_minimal()
